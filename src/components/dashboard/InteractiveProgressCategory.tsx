...
const InteractiveProgressCategory = ({
  category,
  onTaskToggle,
  onQaStatusChange,
  onAddPartClick,
  showAddPart = true,
}: InteractiveProgressCategoryProps) => {
  ...
  const isReadyForQa =
    Boolean(onQaStatusChange) && category.requiresQa && completedTasks === totalTasks;

  const getStatusIcon = (status: SubTask['status']) => {
    switch (status) {
      case 'Completed':
        return <CheckCircle2 className="h-5 w-5 text-green-400 mr-3 flex-shrink-0" />;
      case 'Awaiting Approval':
        return <Clock className="h-5 w-5 text-yellow-400 mr-3 flex-shrink-0" />;
      case 'In Progress':
        return <Construction className="h-5 w-5 text-blue-400 mr-3 flex-shrink-0" />;
      default:
        return <Circle className="h-5 w-5 text-gray-500 mr-3 flex-shrink-0" />;
    }
  };
        ...
        <div className="flex-grow">
          <div className="flex justify-between items-center mb-2">
            <h3 className="text-lg font-semibold text-white flex items-center">
              {category.name}
              {category.requiresQa && (
                <span title="This category can use QA when enabled">
            ...
            <span className="text-sm font-medium text-gray-400">
              {weightedTotal > 0
                ? `${Math.round((weightedDone / weightedTotal) * 100)}% of phase (${weightedDone}/${weightedTotal} pts)`
                  ...
                  {category.subTasks.map((task) => (
                    <li key={task.id} className="py-3">
                      ...
                      >
                        ...
                        <span className={`text-gray-300 min-w-0 ${task.status === 'Completed' ? 'line-through text-gray-500' : ''}`}>
                          {task.name}
                      ...
                      {showAddPart && (
                        <div className="pl-8 pt-2">
                          <div className="flex items-center justify-between">
                            <h4 className="text-xs font-semibold text-gray-400 uppercase tracking-wider flex items-center">
                            ...
                            {onAddPartClick && (
                              <button
                                type="button"
                                onClick={() => onAddPartClick(task.id, category.id)}
                            ...
                            <ul className="mt-2 space-y-1">
                              {task.parts.map((part) => (
                                <li key={part.id} className="flex justify-between items-center bg-gray-900/50 p-2 rounded">
                                  ...
                                  <span className={`px-2 py-0.5 text-xs rounded-full ${getPartStatusColor(part.status)}`}>
                                    {part.status}
                                  </span>
                                </li>
                              ))}
