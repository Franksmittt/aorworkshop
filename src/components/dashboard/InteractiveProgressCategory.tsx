                  ...
                  {category.subTasks.map(task => (
                    <li key={task.id} className="py-3">
                      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between">
                        <div onClick={() => onTaskToggle(task.id, category.id)} className="flex items-center cursor-pointer mb-2 sm:mb-0 flex-grow min-w-0">
                          ...
                          <span className={`text-gray-300 min-w-0 ${task.status === 'Completed' ? 'line-through text-gray-500' : ''}`}>
                            ...
                            {task.progressWeight != null && task.progressWeight > 0 && (
                              <span className="ml-2 shrink-0 text-xs font-semibold text-amber-300/90">({task.progressWeight}% of job)</span>
                            )}
                          </span>
                        </div>
