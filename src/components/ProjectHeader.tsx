...
const ProjectHeader = ({ project, overallProgress, onEdit }: ProjectHeaderProps) => {
  const { car, customerName, status, holdReason, createdAt } = project;
  const bookedInLabel = (() => {
    try {
      return format(parseISO(createdAt), "d MMM yyyy 'at' HH:mm");
    } catch {
      return createdAt;
    }
  })();
  const kmLabel = car.mileageIn != null ? `${car.mileageIn.toLocaleString()} km` : null;
    ...
    >
      <div className="flex flex-col md:flex-row justify-between items-start mb-6">
        <div>
          ...
          <p className="text-caption text-[var(--system-gray)] mt-2 space-y-0.5">
            <span className="block">Booked in: {bookedInLabel}</span>
            {kmLabel && <span className="block">Odometer in: {kmLabel}</span>}
          </p>
        </div>
