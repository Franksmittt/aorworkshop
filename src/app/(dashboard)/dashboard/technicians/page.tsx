...
import { getProjects, getShifts, getTechnicians, addTechnician, updateTechnician, deleteTechnician, getUserTimeTrackingStatus } from '@/lib/data-service';
...
import TechnicianFormModal from '@/components/dashboard/TechnicianFormModal';
import ConfirmationModal from '@/components/ui/ConfirmationModal';
import Button from '@/components/ui/Button';
import { Plus, LucideIcon, Wrench, Coffee, LogOut } from 'lucide-react';
...
export default function TechniciansPage() {
  ...
  const handleOpenFormModal = (tech: Technician | null) => {
  ...
  const handleSaveTechnician = (techData: Partial<Technician>) => {
    if (techData.id) {
      updateTechnician(techData.id, techData);
    } else {
      addTechnician(techData as Omit<Technician, 'id' | 'userId'>);
    }
  ...
  return (
    <>
      <div>
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-8">
          <div>
            ...
            <p className="text-gray-400">The human-resource and performance hub for your team.</p>
          </div>
          <div className="flex items-center space-x-4 mt-4 sm:mt-0">
            <div className="flex items-center space-x-2 bg-gray-800 border border-white/10 rounded-lg p-1">
            ...
            <Button onClick={() => handleOpenFormModal(null)} variant="primary" size="sm">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6 mb-8">
          {performanceData.map(techStats => {
            const tech = allTechnicians.find(t => t.id === techStats.technicianId);
            if (!tech) return null;
            return (
              <TechnicianPerformanceCard 
                ...
                onDelete={() => handleOpenDeleteModal(tech)}
              />
            )
          })}
        </div>
      ...
      <ConfirmationModal
        ...
        message={`Are you sure you want to delete ${selectedTechnician?.name}? This action cannot be undone.`}
      />
    </>
  );
}